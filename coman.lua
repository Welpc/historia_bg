-- ============================================================
--  StarterPlayerScripts_Executor_Scanner_UI_v6.lua
--  Copia: TextBox visible para seleccionar y copiar en Studio
--         setclipboard para Delta/KRNL/Synapse
-- ============================================================
...
-- ----------------------------------------------------------------
-- Ventana de texto para copiar (se abre al presionar Copiar)
-- ----------------------------------------------------------------
local copyWindow = Instance.new("Frame", screenGui)
copyWindow.Name = "CopyWindow"
copyWindow.Size = UDim2.new(0, 400, 0, 300)
copyWindow.Position = UDim2.new(0.5, -200, 0.5, -150)
...
local cwHdrPatch = Instance.new("Frame", cwHdr)
cwHdrPatch.Size = UDim2.new(1, 0, 0.5, 0)
cwHdrPatch.Position = UDim2.new(0, 0, 0.5, 0)
cwHdrPatch.BackgroundColor3 = C.header
cwHdrPatch.BorderSizePixel = 0
cwHdrPatch.ZIndex = 31

label(cwHdr, {
	text  = "Reporte — Selecciona todo con Ctrl+A luego Ctrl+C",
	font  = Enum.Font.GothamBold,
	size  = 11,
	color = C.textMuted,
...
local cwClose = Instance.new("TextButton", cwHdr)
cwClose.Size = UDim2.new(0, 26, 0, 26)
cwClose.Position = UDim2.new(1, -32, 0.5, -13)
cwClose.BackgroundColor3 = C.btnClose
cwClose.Text = "X"
cwClose.TextColor3 = C.white
cwClose.TextSize = 11
...
-- TextBox con el reporte (editable para poder seleccionar)
local reporteTextBox = Instance.new("TextBox", copyWindow)
reporteTextBox.Size = UDim2.new(1, -16, 1, -50)
reporteTextBox.Position = UDim2.new(0, 8, 0, 44)
reporteTextBox.BackgroundColor3 = C.panel
reporteTextBox.BorderSizePixel = 0
reporteTextBox.Text = ""
reporteTextBox.TextColor3 = C.text
reporteTextBox.TextSize = 11
reporteTextBox.Font = Enum.Font.Code
reporteTextBox.MultiLine = true
reporteTextBox.TextWrapped = true
reporteTextBox.TextXAlignment = Enum.TextXAlignment.Left
reporteTextBox.TextYAlignment = Enum.TextYAlignment.Top
reporteTextBox.ClearTextOnFocus = false
reporteTextBox.ZIndex = 32
corner(reporteTextBox, 8)
stroke(reporteTextBox, C.border, 1)

local tbPad = Instance.new("UIPadding", reporteTextBox)
...
copyBtn.MouseButton1Click:Connect(function()
	...
	-- Intento 1: setclipboard (Delta, KRNL, Synapse, Fluxus)
	...
	-- Intento 2: toclipboard (Arceus X)
	if toclipboard then
	...
	-- Fallback: abrir ventana con TextBox visible para Ctrl+A, Ctrl+C
	reporteTextBox.Text = texto
	copyWindow.Visible = true
	task.wait(0.1)
	reporteTextBox:CaptureFocus()
	-- Seleccionar todo el texto automáticamente
	reporteTextBox.SelectionStart = 1
	reporteTextBox.CursorPosition = #texto + 1
end)
